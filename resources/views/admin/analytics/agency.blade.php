...
@section('title', '管理画面 [' . $detail['agency']['name'] . ' の分析]')

@section('content')
<section class="lma-content flex">
    <div class="lma-main_head">
        <div class="lma-title_block">
            <h2>{{ $detail['agency']['name'] }}</h2>
            <p class="analytics_agency">加盟サロン {{ number_format(count($detail['salons'])) }} 件</p>
        </div>
    </div>
        ...
        <div class="record_block">
            ...
            <div class="records_table">
                <ul class="lma-pnavi_list clearfix analytics_pnavi">
                    <li class="prev"><a href="{{ route('admin.analytics.agency', ['agency' => $detail['agency']['id'], 'month' => $prevMonth]) }}">先月</a></li>
                    {{-- 当月を表示中は次月へ進めない --}}
                    @if ($hasNextMonth)
                    <li class="next"><a href="{{ route('admin.analytics.agency', ['agency' => $detail['agency']['id'], 'month' => $nextMonth]) }}">次月</a></li>
                    @endif
                </ul>
                <table class="lma-detail_tbl analytics_tbl">
                    <tbody>
