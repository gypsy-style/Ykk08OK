...
<section class="lma-content flex">
    ...
    <div class="lma-content_block store_edit">
        <form action="{{ route('agencies.merchants.update', $merchant->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <dl class="lma-form_box">
                ...
                <dd>
                    <select class="form-control" id="status" name="status">
                        <option value="1"  {{ $merchant->status == 1 ? 'selected' : '' }}>承認</option>
                        <option value="2"  {{ $merchant->status == 2 ? 'selected' : '' }}>非承認</option>
                    </select>
                </dd>
                ...
                <dd><textarea class="form-control" id="bank_account_name" name="bank_account_name" rows="4">{{ old('bank_account_name', $merchant->bank_account_name) }}</textarea></dd>
