...
**督促対象は「テストを含めない」チェックボックスに連動する。**
売上管理の一覧自体が同チェックボックスに従ってテスト加盟店を除外するため、見えているものと送信対象を一致させる。
テスト加盟店（`merchants.is_test = 1` または所属代理店の `agencies.is_test = 1`）を督促対象に含めたい場合は、
チェックボックスを外してからページを表示し、その状態で督促を実行すること。
